...
<Section>
      ...
      <tbody>
        {orderDetails.length > 0 ? (
          orderDetails.map((detail) => {
            const quantity = detail.PURCHASE_ORDER_DET_PROD_LINE_QTY || 0;
            const purchasePrice = parseFloat(detail.PURCHASE_ORDER_DET_PROD_PURCHASE_PRICE) || 0;
            const sellPrice = parseFloat(detail.PURCHASE_ORDER_DET_PROD_SELL_PRICE) || 0;
            const discount = parseFloat(detail.PURCHASE_ORDER_DET_PROD_DISCOUNT) || 0;

            // Calculate total per row: (Sell Price * Quantity) - Discount Amount
            const discountValue = (discount / 100) * sellPrice * quantity;
  ...
  <TotalSection>
    <TotalRow style={{ display: "flex", justifyContent: "flex-end" }}>
      ...
      <TotalValue>
        {orderDetails.reduce((acc, detail) => acc + (parseInt(detail.PURCHASE_ORDER_DET_PROD_LINE_QTY, 10) || 0), 0)}
      </TotalValue>
    </TotalRow>
    ...
    <TotalRow style={{ display: "flex", justifyContent: "flex-end" }}>
      ...
      <TotalValue>
        ...
        {orderDetails.reduce((acc, detail) => {
          const discountValue =
            (((parseFloat(detail.PURCHASE_ORDER_DET_PROD_SELL_PRICE) || 0) *
              (parseFloat(detail.PURCHASE_ORDER_DET_PROD_DISCOUNT) || 0)) / 100) *
            (parseInt(detail.PURCHASE_ORDER_DET_PROD_LINE_QTY, 10) || 0);
    ...
    <TotalRow style={{ display: "flex", justifyContent: "flex-end" }}>
      ...
      <TotalValue style={{ color: "#ff5757" }}>
        ...
        {orderDetails.reduce((acc, detail) => {
          const totalCost =
            (parseFloat(detail.PURCHASE_ORDER_DET_PROD_PURCHASE_PRICE) || 0) *
            (parseInt(detail.PURCHASE_ORDER_DET_PROD_LINE_QTY, 10) || 0);
